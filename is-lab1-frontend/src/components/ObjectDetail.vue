  ...
  <div class="modal">
    <div class="card">
      ...
      <div v-if="item">
        ...
        <div><strong>Создан</strong> {{new Date(item.creationDate).toLocaleString()}}</div>


        <div style="margin-top:12px">
          <h4>Связанные объекты</h4>
          <div class="small-muted">Координаты: {{item.coordinates?.id}} (x={{item.coordinates?.x}}, y={{item.coordinates?.y}})</div>
          <div class="small-muted">Режиссёр: {{item.director?.id}} — {{item.director?.name}}</div>
          <div class="small-muted">Сценарист: {{item.screenwriter?.id}} — {{item.screenwriter?.name}}</div>
          <div class="small-muted">Оператор: {{item.operator?.id}} — {{item.operator?.name}}</div>
        </div>
      </div>
