        ...
        <div v-for="f in fields" :key="f.key" class="form-field" :style="f.full ? 'grid-column:1/3' : ''">
          ...
          <template v-else>
            <input
              ...
              :type="f.type || 'text'"
              :value="form[f.key]"
              @input="e => form[f.key] = isNumber(f.type) ? toNumber(e.target.value) : e.target.value"
            />
          </template>
...
export default {
  ...
  setup(props, { emit }){
    ...
    const defaults = () => Object.fromEntries(props.fields.map(f => [f.key, f.default ?? null]))
    form.value = defaults()

    const load = async () => {
      if (props.mode === 'edit' && props.id) {
        try { form.value = props.fromEntity(await getOne(props.resource, props.id)) }
    ...
    const validate = () => {
      ...
      for (const f of props.fields) {
        if (f.required && (form.value[f.key] === null || form.value[f.key] === '' || typeof form.value[f.key] === 'undefined')) {
